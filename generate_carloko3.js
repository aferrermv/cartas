...
const manualMap = {
    ...
    // We will add more if the first run fails
};
...
function findOriginalCard(filename) {
    ...
    found = sortedCartas.find(c => {
        ...
        // Only if cleanBase is meaningful
...
console.log(`\n🃏 Generando lote Carloko Nuevo - Medio — ${files.length} archivos desde ID ${START_ID}...\n`);
...
files.forEach(file => {
    ...
    if (original) {
        ...
        newCard.tier = "medio"; // As requested
...
fs.writeFileSync('d:/cartas/cartasCarloko3.json', JSON.stringify(results, null, 2));
console.log(`\n📄 Archivo generado: cartasCarloko3.json (${results.length} cartas)`);
